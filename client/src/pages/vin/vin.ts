...
//        if (platforms.indexOf("core")!=-1) {
            ...
            let url = prefix+'GWS/'+this.cleanForUrl(this.vin.origine.region)+'/'+this.cleanForUrl(this.vin.nom)+'/'+this.vin.annee;
            this.logger.log("[Vin.getGWSScore]url :"+url);
            this.http.get(url)
                    .map(res => res.json())
                    .subscribe((GWSScore:any) => {
...
/*        } else {
            // Create url
            let prefix = '/api/';
            //let prefix = 'http://www.globalwinescore.com/wine-score/';
